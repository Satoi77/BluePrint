...
def _dir_key(file_path: str) -> str:
    """功能单元 = 文件所在目录；顶层文件归入 (root)。"""
    return file_path.rsplit("/", 1)[0] if "/" in file_path else "(root)"


def aggregate_by_function(
    nodes: list[NodeModel], edges: list[EdgeModel]
) -> tuple[list[NodeModel], list[EdgeModel]]:
    """把文件级图聚合为功能级图：一个目录/包 = 一个功能节点。

    功能节点保留 `files`（包含的文件）供悬停展示；状态与提交取该功能内最近一次提交。
    """
    buckets: dict[str, list[NodeModel]] = {}
    for node in nodes:
        buckets.setdefault(_dir_key(node.file_path), []).append(node)

    fn_edges: list[EdgeModel] = []
    seen: set[tuple[str, str]] = set()
    in_degree: dict[str, int] = {}
    out_degree: dict[str, int] = {}
    for edge in edges:
        source = _dir_key(edge.source)
        target = _dir_key(edge.target)
        if source == target:
            continue
        key = (source, target)
        if key in seen:
            continue
        seen.add(key)
        fn_edges.append(EdgeModel(source=source, target=target))
        out_degree[source] = out_degree.get(source, 0) + 1
        in_degree[target] = in_degree.get(target, 0) + 1

    fn_nodes: list[NodeModel] = []
    for dir_key in sorted(buckets):
        members = buckets[dir_key]
        latest: Optional[NodeModel] = None
        ...
        for member in members:
            ...
            if latest_dt is None or committed > latest_dt:
                latest_dt = committed
                latest = member

        files = sorted(member.file_path for member in members)
        functions: list[str] = []
        seen_fn: set[str] = set()
        for member in members:
        ...
        if latest is not None:
            ...
            last_time = latest.last_commit_time
            last_hash = latest.last_commit_hash
            last_message = latest.last_commit_message
        else:
            status = "uncommitted"
            last_time = None
            last_hash = None
        ...
        is_isolated = (
            in_degree.get(dir_key, 0) == 0 and out_degree.get(dir_key, 0) == 0
        )
        fn_nodes.append(
            NodeModel(
                id=dir_key,
